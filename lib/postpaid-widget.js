...
const toCollectUrl = ({ id, lang }) => `${origin}/${lang}/app/collect/${id}`;

const toCauseImageUrl = (cause) =>
  `${origin}/donateeGroup/${cause}.v0000001.png`;

const toApiEndpointUrl = ({ simulate }) =>
  `${origin}/api/donate/postpaid${simulate ? "?simulate=true" : ""}`;
...
const Api = {
  donatePostpaid: ({
    ...
    fetch(toApiEndpointUrl({ simulate }), {
      method: "POST",
...
const renderSlot = () => html`<slot></slot>`;

// WEB COMPONENT

const ThankUPostpaidWidget = {
  lang: "en",
  slug: "",
  cause: "",
  impact: 0,
  ...
  data: ({ lang, slug, cause, impact, pid, sig, message, simulate }) =>
    Api.donatePostpaid({
      language: lang,
      slug,
      donateeGroup: cause,
  ...
  render: ({ data, lang, cause, impact, message }) => {
    const t = translations[lang] || translations.en;
    const promise = data
      .then(({ id }) => renderData({ id, cause, impact, message, t, lang }))
      .catch(renderSlot);
    return html.resolve(promise, renderSlot());
  },
};

define("thanku-postpaid-widget", ThankUPostpaidWidget);
